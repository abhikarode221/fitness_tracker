...
export const History = () => {
  ...
  const fetchHistory = async () => {
    try {
      ...
      const res = await axios.get('http://localhost:5000/api/workouts/all', {
  ...
  const commitUpdate = async () => {
    try {
      const token = localStorage.getItem('kinetic_token');
      await axios.put(`http://localhost:5000/api/workouts/${editTarget._id}`, 
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-12 pb-20">
      <header>
        <h1 className="text-5xl font-black text-white tracking-tighter uppercase italic">Session History</h1>
        ...
        {workouts.length > 0 ? (
          workouts.map((workout) => (
            <motion.div 
              ...
              className="bg-zinc-900/40 border border-white/5 p-8 rounded-[2.5rem] backdrop-blur-3xl group"
            >
              <div className="flex justify-between items-start mb-6">
                <div className="flex items-center gap-4">
                  <div className="p-3 bg-zinc-800 rounded-2xl text-zinc-500"><Calendar size={20} /></div>
                  <div>
                    <h3 className="text-white font-bold text-xl uppercase italic tracking-tight">
                ...
                <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => setEditTarget(JSON.parse(JSON.stringify(workout)))} className="p-3 bg-white/5 hover:bg-cyan-400 hover:text-black text-zinc-500 rounded-xl transition-all"><Edit3 size={18} /></button>
                  <button onClick={() => handlePurge(workout._id)} className="p-3 bg-white/5 hover:bg-red-500 hover:text-white text-zinc-500 rounded-xl transition-all"><Trash2 size={18} /></button>
                </div>
              </div>
          ...
          <div className="text-center py-20 bg-zinc-900/20 border border-dashed border-white/5 rounded-[3rem]">
      ...
      <AnimatePresence>
        {editTarget && (
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-6">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative w-full max-w-2xl bg-zinc-950 border border-white/10 p-10 rounded-[3rem] shadow-2xl max-h-[80vh] overflow-y-auto custom-scrollbar">
              <div className="flex justify-between items-center mb-10">
                <h2 className="text-2xl font-black text-white uppercase italic tracking-tighter">Recalibrate Session</h2>
              ...
              <div className="space-y-8">
                {editTarget.exercises.map((ex, eIdx) => (
                  <div key={eIdx} className="space-y-4 bg-white/5 p-6 rounded-3xl">
                    <h4 className="text-white font-bold uppercase text-sm tracking-widest">{ex.name}</h4>
                    {ex.sets.map((set, sIdx) => (
              ...
              <button onClick={commitUpdate} className="w-full mt-10 bg-cyan-400 text-black font-black py-5 rounded-2xl uppercase tracking-widest text-xs flex items-center justify-center gap-2">
