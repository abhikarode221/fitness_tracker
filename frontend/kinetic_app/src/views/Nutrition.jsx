...
export const Nutrition = () => {
  ...
  const fetchAllData = async () => {
    try {
      ...
      const fuelRes = await axios.get('http://localhost:5000/api/nutrition/today', { headers });
      setLog(fuelRes.data);
  ...
  const deleteMeal = async (mealId) => {
    try {
      ...
      const res = await axios.delete(
        `http://localhost:5000/api/nutrition/meal/${mealId}`,
        { headers: { 'x-auth-token': token } }
      );
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-10 pb-20">
      ...
      <header className="flex justify-between items-end">
        <div>
          <h1 className="text-5xl font-black text-white tracking-tighter uppercase italic">Fuel Center</h1>
          <p className="text-zinc-500 font-mono text-[10px] uppercase tracking-[0.4em] mt-2">Precise Intake Tracking</p>
        </div>
        <button
          onClick={() => setIsModalOpen(true)}
          className="flex items-center gap-2 bg-cyan-400 text-black font-black px-8 py-3 rounded-xl uppercase tracking-widest text-xs shadow-[0_0_20px_cyan]"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-zinc-900/40 border border-white/5 p-10 rounded-[2.5rem] backdrop-blur-3xl">
          <p className="text-zinc-500 font-mono text-[10px] uppercase tracking-[0.3em] mb-6">Energy Intake</p>
          <div className="flex items-baseline gap-4">
            <h2 className="text-8xl font-black text-white tracking-tighter">{totals.calories}</h2>
            <span className="text-zinc-600 font-mono text-sm uppercase">/ {activeCalorieGoal} KCAL</span>
          </div>

          <div className="w-full h-3 bg-zinc-800 rounded-full mt-8 overflow-hidden">
            <motion.div
              initial={{ width: 0 }}
              animate={{
        ...
        <div className="bg-zinc-900/40 border border-white/5 p-10 rounded-[2.5rem] flex flex-col justify-between">
          <div>
            <p className="text-zinc-500 font-mono text-[10px] uppercase tracking-[0.3em] mb-6">Hydration</p>
            <h2 className="text-6xl font-black text-white tracking-tighter">{log.waterIntake.toFixed(2)} L</h2>
          </div>
          <div className="grid grid-cols-2 gap-4 mt-12">
            <button onClick={() => updateWater(0.25)} className="bg-white/5 py-4 rounded-xl text-[10px] font-bold text-zinc-400 uppercase">+250ml</button>
            <button onClick={() => updateWater(0.5)} className="bg-white/5 py-4 rounded-xl text-[10px] font-bold text-zinc-400 uppercase">+500ml</button>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {log.meals.map(meal => (
          <div key={meal._id} className="bg-zinc-900/40 border border-white/5 p-6 rounded-[2rem] flex justify-between items-center group">
            <div className="flex gap-6 items-center">
               <div className="w-12 h-12 bg-zinc-800 rounded-xl flex items-center justify-center text-zinc-500"><Utensils size={20} /></div>
               <div><h4 className="text-white font-bold text-xl">{meal.name}</h4><p className="text-zinc-600 font-mono text-[9px] uppercase">{meal.time}</p></div>
            </div>
            <div className="flex gap-8 items-center">
              <p className="text-cyan-400 font-black text-xl">{meal.calories} KCAL</p>
              <button onClick={() => deleteMeal(meal._id)} className="p-2 text-zinc-700 hover:text-red-500 transition-colors">
                <Trash2 size={20} />
              </button>
            </div>
          </div>
      ...
      <AnimatePresence>
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative bg-zinc-950 border border-white/10 p-10 rounded-[3rem] w-full max-w-md">
              <div className="flex justify-between items-center mb-8">
                <h2 className="text-2xl font-black text-white uppercase italic tracking-tighter">Log Fuel</h2>
              ...
              <div className="space-y-4">
                ...
                <button onClick={handleAddMeal} className="w-full bg-cyan-400 py-4 rounded-2xl text-black font-black uppercase text-xs tracking-widest mt-4">Save Meal</button>
              </div>
            </motion.div>
          </div>
        )}
